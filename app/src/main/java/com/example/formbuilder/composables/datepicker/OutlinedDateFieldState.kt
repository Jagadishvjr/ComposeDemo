package com.example.formbuilder.composables.datepicker

import androidx.compose.runtime.mutableStateOf
import androidx.compose.runtime.saveable.rememberSaveable
import androidx.compose.runtime.*
import androidx.compose.ui.geometry.Size
import com.example.composedemo.R
import com.example.formbuilder.BaseState
import com.example.formbuilder.TextFieldState
import com.example.formbuilder.Transform
import com.example.formbuilder.Validators


class OutlinedDateFieldState(){

        var enabled by mutableStateOf(false)
        var value by mutableStateOf("")
        var label by mutableStateOf("DOB")
        var selectedIndex by mutableStateOf(-1)
        var size by mutableStateOf(Size.Zero)
        val items = (1..11).map{
            if(it == 1){
                "Select"
            }else{
                "Option ${it-1}"
            }

        }


        fun onEnabled(newValue:Boolean){
            enabled = newValue
        }
        fun onSelectedIndex(newVal:Int){
            selectedIndex = newVal
            value = items[selectedIndex]
        }
        fun onSize(newVal:Size){
            size = newVal
        }
    }


@Composable
fun remeberOutlinedDateFieldState() = remember { //TODO chenge remember to rememberSavable
    OutlinedDateFieldState()
}