...
open class TextFieldState(
    ...
    override fun validate(): Boolean {
        val validations = validators.map {
            Log.d("Validators","validators- $it")

            when (it) {
                is Validators.Email -> validateEmail(it.message)
                is Validators.Phone -> validatePhoneNumber(it.message)
        ...
        val finalDecision = validations.all { it }
    ...
    /**
     *This function validates an Phone number in [value]
     *It will return true if the string value is a valid phone number.
     *The implementation makes use of the [android.util.Patterns] class to match the phone address.
    ...
    private fun validatePhoneNumber(message: String): Boolean {
        val valid = Patterns.PHONE.matcher(value).matches()
        if (!valid) showError(message)
        return valid
    }
